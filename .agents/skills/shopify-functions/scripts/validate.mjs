...
function findVersionedSchemaInBundledAssets(apiName2, version) {
  const __filename = fileURLToPath2(import.meta.url);
  const __dirname = path2.dirname(__filename);
  const assetsDir = path2.join(__dirname, "..", "assets");
  const gz = path2.join(assetsDir, `${apiName2}_${version}.json.gz`);
  if (existsSync2(gz)) return gz;
  const json = path2.join(assetsDir, `${apiName2}_${version}.json`);
  if (existsSync2(json)) return json;
  const available = readdirSync(assetsDir).filter((f) => f.startsWith(`${apiName2}_`)).join(", ");
  throw new Error(
    `Schema for '${apiName2}' version '${version}' is not bundled with this skill. Available: ${available || "none"}.`
  );
}
function resolveSchemaPath(apiName2, version) {
  if (true) {
    return findVersionedSchemaInBundledAssets(apiName2, version);
  }
  return loadAPISchema(apiName2, { name: version, latestVersion: false }).schemaPath;
}
